...
export async function planSpec(
  ...
  // CC-58（jason 12:13）：planner 按 key 价值级联，用尽即切下一个。默认 workers-ai（含额度）后
  // 依次 deepseek-chat → hilinkup。仍不把 claude 放进默认链（本机订阅，云端无 login 一触发即报错）。
  const primary = config.providers.planner;
  const fallbacks = (process.env.LOOP_PLANNER_FALLBACK ?? "deepseek-chat,hilinkup:kimi-k2.5")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
